...
namespace Michsky.DreamOS
{
    public class GameHubManager : MonoBehaviour
    {
        ...
        // Downloads
        public Animator downloadsButton;
        ...
        void InitializeGames()
        {
            ...
            {
                if (libraryAsset.games[i].gameDownloaded == true && PlayerPrefs.HasKey(libraryAsset.games[i].gameTitle + "Downloaded") == false)
                    PlayerPrefs.SetString(libraryAsset.games[i].gameTitle + "Downloaded", "true");
                ...
                if (PlayerPrefs.GetString(libraryAsset.games[i].gameTitle + "Downloaded") == "true")
            ...
            for (int i = 0; i < libraryAsset.games.Count; ++i)
            {
                ...
                downloadedIndicator = go.transform.Find("Content/Downloaded").gameObject;
        ...
        public void CheckForGameInformation(string gameName)
        {
            for (int i = 0; i < libraryAsset.games.Count; ++i)
            {
                if (libraryAsset.games[i].gameTitle == gameName)
                {
                    ...
                    if (PlayerPrefs.GetString(gameName + "Downloaded") == "true")
                        currentDownloadState = "true";
                    else if (PlayerPrefs.GetString(gameName + "Downloaded") == "false")
        ...
        public void DownloadGame(string gameName)
        {
            ...
            GameHubDownloadItem itemComp = downloadItemGO.GetComponent<GameHubDownloadItem>();
        ...
        {
            PlayerPrefs.SetString(gameName + "Downloaded", "true");
            OpenGamePage(gameName, true, true);

            // For store item
            GameObject downloadIndicator;
            downloadIndicator = storeItemParent.Find(gameName).Find("Content/Downloaded").gameObject;
        ...
        public void UninstallGame(string gameName)
        {
            ...
            // For store item
            PlayerPrefs.SetString(gameName + "Downloaded", "false");
            storeItemParent.Find(gameName).Find("Content/Downloaded").gameObject.SetActive(false);
        ...
        IEnumerator WaitForSliderTimer()
        {
            ...
            if (currentSliderIndex == sliderItems.Count - 1)
